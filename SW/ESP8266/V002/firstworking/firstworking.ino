...
const char* mqtt_server = "10.0.22.139";
...
void loop() {
  ...
  if (now - lastMeasure > 1000) {
    ...
    static char co2_ppmCO2[7];
    dtostrf(co2_ppm, 6, 2, co2_ppmCO2);

    static char voc_ppbVOC[7];
    dtostrf(voc_ppb, 6, 2, voc_ppbVOC);

    // Publishes Temperature and Humidity values
    client.publish("room/temp", temperatureTemp);
    client.publish("room/humidity", humidityHum);
    client.publish("room/MacAddr", macaddr ,6);
    client.publish("room/co2", co2_ppmCO2);
    client.publish("room/voc", voc_ppbVOC);
