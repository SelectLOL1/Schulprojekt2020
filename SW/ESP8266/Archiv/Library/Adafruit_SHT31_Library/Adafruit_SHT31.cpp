...
uint16_t Adafruit_SHT31::readStatus(void) {
  writeCommand(SHT31_READ_STATUS);
...
void Adafruit_SHT31::reset(void) {
  writeCommand(SHT31_SW_RESET);
  delay(10);
}

void Adafruit_SHT31::heater(boolean on) {
  if (on)
    writeCommand(SHT31_HEATER_ENABLE);
  else
    writeCommand(SHT31_HEATER_DISABLE);
}
